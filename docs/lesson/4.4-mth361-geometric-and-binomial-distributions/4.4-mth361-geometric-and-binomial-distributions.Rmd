...
## Bernoulli, Geometric, and Binomial Distribution Summary

| $X$ | **Bernoulli** | **Geometric** | **Binomial** |
|:---|:------:|:------:|:------:|
...
| *Parameters* | $p \longrightarrow$ probability of "success" | $p \longrightarrow$ probability of "success" | $n \longrightarrow$ number of trials<br>$p \longrightarrow$ probability of "success" |
| *PMF*<br>$P(X=k)$ | $p^k (1-p)^{1-k}$<br>$\text{for } k = 0,1$ | $(1-p)^k p$<br>$\text{for } k=0,1,2,\cdots$ | $\binom{n}{k} p^k (1-p)^{n-k}$<br>$\text{for } k = 0,1,2, \cdots, n$ |
| *Expected Value*<br>$\text{E}(X)$ | $p$ | $\frac{1-p}{p}$ | $np$ |
| *Variance*<br>$\text{Var}(X)$ | $p(1-p)$ | $\frac{1-p}{p^2}$ | $np(1-p)$ |
| $P(X = k)$ | `dbinom(1,p)` | `dgeom(p)` | `dbinom(n,p)` |
| $P(X \le k)$ | `pbinom(1,p)` | `pgeom(p)` | `pbinom(n,p)` |
| $k$ | `qbinom(P,1,p)` | `qgeom(P,p)` | `qbinom(P,n,p)` |
| $N$ *Simulations* | `rbinom(N,1,p)` | `rgeom(N,p)` | `rbinom(N,n,p)` |
...
## Why the Geometric and Binomial Distributions are Usefull?
