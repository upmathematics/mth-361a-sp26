...
p3 <- ggplot(df,aes(x=n,y=prop,color=X,group=X)) + 
  ...
  scale_color_manual(values=c(`H`="#009159")) + 
  geom_hline(aes(yintercept = p), color = "#f1cc54", linetype="dashed", linewidth=1) + 
  geom_text(aes(0,p),label = paste(TeX("P(H)")," = ",p,sep=""), 
            vjust = -1, hjust=-1.75,color="#f1cc54") + 
  ylim(0,1) + 
  xlab("n") + 
  ylab("proportion") + 
  ggtitle("Simulation of Cumulative Fair Coin Flips") + 
  theme_minimal() + 
...
animate(p3_anim, nframes = round(length(n)/5), 
        renderer = gifski_renderer("cumulative-fair-coin-flips-sim.gif"),
