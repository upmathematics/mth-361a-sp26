...
The Bernoulli R.V. models key scenarios used in:

:::: {.column width=15%}
::::
 
:::: {.column width=70%}
* *Medical trials*: success or failure of clinical trials
* *Quality control*: defective or not defective product
* *Finance*: default or no default of a loan
* *Machine learning*: building a binary classification model
* $\cdots$ etcetera
::::
...
::: {style="color: red;"}
$\star$ It is the baseline of binary data modeling.
:::
...
Assume the probability that the test is positive is $p = 0.12$.

Then, $$X \sim \text{Bern}(0.12).$$
...
::: {style="color: red;"}
$\star$ This is a Bernoulli r.v. because there are exactly two possible outcomes, with a set "positive" test probability $p$.
:::
...
* This statement refers only to the test result, not whether the individual actually has the disease, since the test’s accuracy is unknown.
::::
